...
function App() {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const apiUrl = isProduction ? '/api/quizzes' : 'http://localhost:8000/api/quizzes';
  ...
  const confirmDelete = async () => {
    ...
    try {
      const isProduction = process.env.NODE_ENV === 'production';
      const apiUrl = isProduction ? `/api/quiz/${quizId}` : `http://localhost:8000/api/quiz/${quizId}`;
  ...
  const handleGenerate = async () => {
    ...
    const isProduction = process.env.NODE_ENV === 'production';
    const apiUrl = isProduction ? '/api/quiz' : 'http://localhost:8000/api/quiz';
  ...
  const handleSaveQuiz = async () => {
    ...
    try {
      const isProduction = process.env.NODE_ENV === 'production';
      const apiUrl = isProduction ? `/api/quiz/${quizData.id}/save-results` : `http://localhost:8000/api/quiz/${quizData.id}/save-results`;
